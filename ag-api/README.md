...
Monolithic Express API for the ag platform. First module: `jobs` (recruitment).
...
`TURNSTILE_SITE_KEY` when running `scripts/generate-static.mjs` in `ag-web`.
...
Since ag-web is a static site, it only reflects reality after a rebuild.
...
event (event_type listing-changed) against ag-web's repo. ag-web's
