...
Jest + Supertest against a **real** Postgres instance (`agjob_ci` db) — no
...
- Farm selection UI: currently every per-module dashboard (just `jobs` so
  ...
  module — this is `core`-level concern, not jobs-specific. See ag-web's
  CLAUDE.md for the frontend side of this.

## Local dev

See `README.md` in this repo, and `LOCAL_SETUP.md` (delivered separately,
covers running ag-api + ag-web together). Postgres native or Docker both
