...
describe("POST /api/core/auth/register", () => {
  it("creates a new user account", async () => {
    const res = await request(app).post("/api/core/auth/register").send({
      email: "farmer@agjob-tests.example",
      password: "supersecret123",
      firstName: "Yoann",
      lastName: "Test",
    });
    expect(res.status).toBe(201);
    expect(res.body.user.email).toBe("farmer@agjob-tests.example");
    expect(res.body.user.password_hash).toBeUndefined();
  });

  it("rejects a password shorter than 8 characters", async () => {
    const res = await request(app).post("/api/core/auth/register").send({
      email: "short@agjob-tests.example", password: "abc", firstName: "A", lastName: "B",
    });
  ...
  it("rejects missing required fields", async () => {
    const res = await request(app).post("/api/core/auth/register").send({ email: "x@agjob-tests.example" });
    expect(res.status).toBe(400);
  });

  it("rejects a duplicate email with 409", async () => {
    const payload = { email: "dup@agjob-tests.example", password: "supersecret123", firstName: "A", lastName: "B" };
...
describe("POST /api/core/auth/login", () => {
  it("returns a JWT for valid credentials", async () => {
    await request(app).post("/api/core/auth/register").send({
      email: "login@agjob-tests.example", password: "supersecret123", firstName: "A", lastName: "B",
    });
    const res = await request(app).post("/api/core/auth/login").send({
      email: "login@agjob-tests.example", password: "supersecret123",
    });
  ...
  it("rejects a wrong password with 401", async () => {
    await request(app).post("/api/core/auth/register").send({
      email: "wrongpw@agjob-tests.example", password: "supersecret123", firstName: "A", lastName: "B",
    });
    const res = await request(app).post("/api/core/auth/login").send({
      email: "wrongpw@agjob-tests.example", password: "totallywrong",
    });
  ...
  it("rejects an unknown email with 401 (not 404 — avoids leaking which emails exist)", async () => {
    const res = await request(app).post("/api/core/auth/login").send({
      email: "doesnotexist@agjob-tests.example", password: "whatever123",
    });
    expect(res.status).toBe(401);
  });
});
