...
Frontend for the ag platform. Two rendering modes living in the same
...
```bash
npm install
cp .env.example .env   # point VITE_API_BASE_URL at a running ag-api
...
`ag-api` must be running (and its DB migrated/seeded) for both `dev` and
...
a Cloudflare Turnstile challenge — see `ag-api`'s README for the server
...
2. **Settings → Secrets and variables → Actions**, add:
   - `API_BASE_URL` — the deployed `ag-api` URL, e.g. `https://ag-api.example.cleverapps.io/api`
